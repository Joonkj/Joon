...
class MS_Token_Grabber:
  ...
  def Grab_Tokens(self, token):
    ...
    try:
      json = {
          'RelyingParty': 'http://accounts.xboxlive.com',
          'TokenType': 'JWT',
          'Properties': {
              'UserTokens': [token],
              'SandboxId': 'RETAIL'
      ...
      if response.status_code == 200:
        token = 'XBL3.0 x={};{}'.format(
            response.json()['DisplayClaims']['xui'][0]['uhs'],
            response.json()['Token'])
...
class Main_Swapper:
      ...
      for token in xbox_tokens:
        ...
        if response.status_code == 409:
          ...
          if self.printed == 0:
            self.printed += 1
            print(
                '{} Successfully Reserved \033[1;32;40m{}\033[1;37;40m After \033[1;32;40m{:,}\033[1;37;40m Attempt(s)!\n'
  ...
  def SendHook(self, gamertag, elapsed):
    ...
    embed.add_embed_field(name="Gamertag",
                          value="`{}`".format(gamertag),
                          inline=False)
    embed.add_embed_field(name="Attempts",
    ...
    while True:
      for token in ms_tokens:
        post = {
            ...
            'userHash': token.split(';')[0].split('=')[1],
            'userKey': None,
            'userXuid': '216258806147975844'
        }
        headers = {'x-xbl-contract-version': '4', 'Authorization': token}
        response = requests.post(
            'https://accountstroubleshooter.xboxlive.com/users/current/profile',
            json=post,
            headers=headers)
        if response.status_code == 400:
          self.attempts += 1
          print('Attempts - {} | RL - {} | R/S - ({:,})'.format(
              self.attempts, self.rl, self.rs),
                end="\r")
          self.RPS_Threading()
        elif response.status_code == 200:
          if response.json()['gamerTag'] == gamertag:
            if self.printed == 0:
              ...
              print(
                  '{} Successfully Claimed \033[1;32;40m{}\033[1;37;40m After \033[1;32;40m{:,}\033[1;37;40m Attempt(s)!\n'
                  .format(swap, gamertag, self.attempts))
